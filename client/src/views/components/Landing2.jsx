...
const HeaderBanner = () => {
  ...
  const [user, setUser] = useState(null); // Renamed setUserID to setUser for clarity
    ...
    axios
      ...
      .then((response) => {
        if (response.data.userData && response.data.userData.userName) {
          setUser(response.data.userData.userName);
        } else if (response.data.userData && response.data.userData.psyName) {
          setUser(response.data.userData.psyName);
        }
      })
      .catch((error) => {
        ...
        <div className="text-black w-[90%] mx-auto">
          <div className="text-white lg:flex lg:justify-between lg:items-center sm:pt-[10px] lg:pt-[0px]">
            <div className="">
              ...
              {(!user || !user.startsWith("Dr.")) && ( // Conditionally render the line based on user
              ...
              {user &&
                user.startsWith("Dr.") && ( // Conditionally render the line based on user
                  <p className="text-2xl font-medium pt-5">Browse the site</p>
                )}
              <div className="">
                <SearchBar />
              </div>
